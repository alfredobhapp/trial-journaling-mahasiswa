...
try {
    ...
    foreach ($entries_raw as $r) {
        $entries[] = [
            ...
            'moods' => (!empty($r['moods']) && is_string($r['moods'])) ? (json_decode($r['moods'], true) ?: []) : [],
            'enthusiasm' => (int)$r['enthusiasm'],
            'burden' => $r['burden'],
            'dosen' => $r['dosen'],
            'hambatan' => (!empty($r['hambatan']) && is_string($r['hambatan'])) ? (json_decode($r['hambatan'], true) ?: []) : [],
            'hambatanPersonal' => (!empty($r['hambatan_personal']) && is_string($r['hambatan_personal'])) ? (json_decode($r['hambatan_personal'], true) ?: []) : [],
            'selfReflection' => (!empty($r['self_reflection']) && is_string($r['self_reflection'])) ? (json_decode($r['self_reflection'], true) ?: []) : [],
            'bodyReactions' => (!empty($r['body_reactions']) && is_string($r['body_reactions'])) ? (json_decode($r['body_reactions'], true) ?: []) : [],
            'socialReactions' => (!empty($r['socialReactions']) && is_string($r['socialReactions'])) ? (json_decode($r['socialReactions'], true) ?: []) : [],
            'helpNeeds' => (!empty($r['help_needs']) && is_string($r['help_needs'])) ? (json_decode($r['help_needs'], true) ?: []) : [],
            'contact' => $r['contact'],
            'ews' => (!empty($r['ews_result']) && in_array($r['ews_result'], ['normal', 'akademik', 'konseling'])) ? $r['ews_result'] : 'normal',
